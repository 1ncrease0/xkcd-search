...
func TestSearch(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, code, "update must complete before search tests")
...
func IndexSearchPhrases(t *testing.T) {
	...
	code, err := update(token)
	require.NoError(t, err, "could not run update")
	require.Equal(t, http.StatusOK, code, "update must complete before isearch tests")
	time.Sleep(30 * time.Second)
