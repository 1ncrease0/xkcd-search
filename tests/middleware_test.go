...
func TestSearchConcurrency(t *testing.T) {
	...
	token := login(t)
	code, err := update(token)
	require.NoError(t, err, "could not update")
	require.Equal(t, http.StatusOK, code, "update must complete before concurrency test")
...
func TestSearchRateLong(t *testing.T) {
	...
	code, err := update(token)
	require.NoError(t, err, "could not update")
	require.Equal(t, http.StatusOK, code, "update must complete before rate test")
...
func TestStatsDbNoToken(t *testing.T) {
...
func TestStatusDbNoToken(t *testing.T) {
...
func TestPingNoToken(t *testing.T) {
	req, err := http.NewRequest(http.MethodGet, address+"/api/ping", nil)
	require.NoError(t, err)
	resp, err := client.Do(req)
	require.NoError(t, err)
	defer resp.Body.Close()
