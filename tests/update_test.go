...
func TestUpdate(t *testing.T) {
	...
	st := stats(t, token)
	require.Equal(t, st.ComicsTotal-1, st.ComicsFetched)
...
func update(token string) (int, error) {
	...
	case http.StatusOK, http.StatusAccepted:
		return code, nil
	default:
		return code, fmt.Errorf("update: unexpected HTTP status %d", code)
	}
}
...
func stats(t *testing.T, token string) UpdateStats {
	req, err := http.NewRequest(http.MethodGet, address+"/api/db/stats", nil)
	require.NoError(t, err, "cannot build stats request")
	req.Header.Add("Authorization", "Token "+token)
	resp, err := client.Do(req)
